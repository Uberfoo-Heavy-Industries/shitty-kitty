...
void SetPixelMode(size_t port, uint8_t mode) {
	...
	HAL_I2C_Master_Transmit(hi2c, PIXEL_CONTROL_ADDR, data, 2, HAL_MAX_DELAY);
}
...
uint8_t GetPixelData(size_t port, uint8_t data) {
	I2C_HandleTypeDef *hi2c = GetSAOI2CPort(port);
	uint8_t buf[] = {data};
	HAL_I2C_Master_Transmit(hi2c, PIXEL_CONTROL_ADDR, buf, 1, HAL_MAX_DELAY);
	HAL_I2C_Master_Receive(hi2c, PIXEL_EEPROM_ADDR, buf, 1, HAL_MAX_DELAY);
	return buf[0];
}
