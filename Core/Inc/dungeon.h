...
typedef struct {
  const char* const * const strs;
  const int n;
} string_group_t;

typedef struct {
  const string_group_t words;
  const char* const inventory;
  int plac, fixd;
  bool is_treasure;
  const char* const * const descriptions;
  const char* const * const sounds;
  const char* const * const texts;
  const char* const * const changes;
} object_t;

typedef struct {
  const char* const small;
  const char* const big;
} descriptions_t;

typedef struct {
  const descriptions_t description;
  const long sound;
  const bool loud;
} location_t;
...
extern const char * const arbitrary_messages[];
...
extern const char * const ignore;

extern const long CONDITIONS_CONST[];
